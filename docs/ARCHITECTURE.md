...
Phase 6 gives each fixed-candidate Phase 2 execution a retry-policy namespace derived from its immutable Phase 2 authority, which already binds the acceptance campaign, candidate, runtime profile, and extractor model. A retry within the same campaign therefore recovers the same checkpoints and permanent terminal without duplicating a provider request, while a later campaign cannot inherit an earlier campaign's exhausted retry budget or permanent terminal. Historical state is retained unchanged. Ordinary discovery continues to use the shared `retry-v1` policy version.
