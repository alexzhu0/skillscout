...
Python 3.12 已进入仅安全修复阶段，而 Python 3.13 的生命周期更长。SkillScout 是绿地项目，不需要为旧运行环境兼容付出成本，因此 MVP 基线改为 Python 3.13；如果目标组织的 runner 或安全基线只允许 3.12，领域代码仍应保持 3.12 兼容，作为部署降级选项。参考 [Python 3.12.13 发布页](https://www.python.org/downloads/release/python-31213/) 与 [Python 3.13 发布计划](https://peps.python.org/pep-0719/)。

### OpenAI Responses API 的使用边界

Structured Outputs 能让模型输出遵循 JSON Schema，并允许代码区分结构化拒绝；Python SDK 可直接与 Pydantic 模型配合。它适合 `WorkflowSpec`、`ReviewDecision` 等语义阶段，但不替代确定性过滤、许可证判断、格式验证或评分规则。参考 [Structured Outputs 指南](https://developers.openai.com/api/docs/guides/structured-outputs)。
...
这些限制来自 OpenAI 对 Prompt Injection、结构化节点边界和人工审批的建议，以及 API 数据控制说明。参考 [Agent safety](https://developers.openai.com/api/docs/guides/agent-builder-safety)、[Safety best practices](https://developers.openai.com/api/docs/guides/safety-best-practices)、[API data controls](https://developers.openai.com/api/docs/guides/your-data) 与 [模型选择指南](https://developers.openai.com/api/docs/guides/latest-model)。

## GitHub API 使用策略

### 读取而非克隆

MVP 只通过 GitHub API 读取元数据和允许的文本文件，不执行 `git clone`、不下载 release artifact、不安装依赖，也不运行仓库脚本。Reader 使用显式 allowlist 和预算：

1. README；
2. `docs/` 中少量高价值文档；
3. `examples/` 中少量文本示例；
4. `package.json`、`pyproject.toml`、`requirements*.txt` 等清单；
5. 最后才读取有限源文件。

GitHub Contents API 对目录最多返回 1,000 项，并对大文件有不同处理规则；SkillScout 自己必须设置远小于平台上限的目录项数、单文件字节数、总字节数和总 token 预算。参考 [Repository contents API](https://docs.github.com/en/rest/repos/contents)。

### 许可证

GitHub Licenses API 使用 Licensee 匹配仓库许可证，但不会替代完整法律审查。MVP 只接受明确识别为 `MIT`、`Apache-2.0`、`BSD-2-Clause` 或 `BSD-3-Clause` 的单一仓库级许可证；`NOASSERTION`、缺失、非标准、多重或冲突许可一律拒绝并给出原因。生成内容必须改写，只保留必要的短证据摘录和归属。参考 [Licenses API](https://docs.github.com/en/rest/licenses)。

### Draft PR

Publisher 只允许创建或更新确定性的发布分支、创建 Draft Pull Request、请求指定人类 Reviewer。它不调用 merge API，不向默认分支 push，不关闭规则集。PR API 支持 `draft` 标志；Review Requests 有独立端点。参考 [Pull Requests API](https://docs.github.com/en/rest/pulls/pulls) 与 [Review requests API](https://docs.github.com/en/rest/pulls/review-requests)。

## GitHub Actions 与持久状态

GitHub-hosted runner 对每个 job 提供新的临时环境，因此把 `skillscout.db` 仅写在工作区会在运行结束后丢失。Actions cache 可能被淘汰且不可变，artifact 又与 workflow run 的保留期绑定，二者都不能作为 canonical state。参考 [GitHub-hosted runners](https://docs.github.com/en/actions/reference/runners/github-hosted-runners)、[Workflow artifacts](https://docs.github.com/en/actions/concepts/workflows-and-actions/workflow-artifacts) 与 [Dependency caching](https://docs.github.com/en/actions/reference/workflows-and-actions/dependency-caching)。
