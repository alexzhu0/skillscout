...
| 06-18-01 | 06-18 | 8 | TEST-01, TEST-02, TEST-04 | Isolated source fixtures close the environment-A benchmark-lock and planned literal environment-B live-authority routes; topology mutations reject missing/cyclic/same/future/wrong-wave plans before workflow bytes change | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_source_execution.py tests/test_phase6_validation_map.py -k 'fixture or protected_authority or dependency_topology or missing_dependency or cycle or wave' -x` | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| Lock the five-repository benchmark | TEST-01 | Repository identity, license, outcome-role mix, and suitability require accountable human selection before credentials or LLM calls | Select and commit the exact static V1 `.planning/phases/06-adversarial-mvp-acceptance/06-BENCHMARK-MANIFEST.json`; configure `phase6-human-benchmark-lock` with required reviewer `alexzhu0`, selected deployment branch `main` only, Prevent self-review disabled for the sole reviewer, no custom protection rule, and `SKILLSCOUT_BENCHMARK_LOCK_STATE_GITHUB_TOKEN` only as an environment secret with no repository-level fallback; approve the state-only lock job so its V2 `benchmark_lock` receipt binds the manifest digest without any comment evidence |
| Approve fresh live authority | TEST-01, TEST-02, TEST-04 | Permission to resolve live credentials is distinct from selecting the benchmark and must be externally auditable | Configure `skillscout-phase6-live-authority` with required reviewer `alexzhu0`, Prevent self-review disabled for the sole reviewer, no custom protection rule, and only its environment-scoped `SKILLSCOUT_LIVE_AUTHORITY_STATE_GITHUB_TOKEN`; approve only the waiting state-only `record-live-authority` job and confirm a V2 `live_execution` receipt bound to the V2 lock, without a comment, actor assertion, semantic call, or publication effect |
