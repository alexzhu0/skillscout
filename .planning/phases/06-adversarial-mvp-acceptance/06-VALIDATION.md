...
All 47 Phase 6 tasks appear exactly once below as primary rows. Checkpoint dependencies are explicit, and every row has an automated command even when human or hosted evidence is also required.
...
| 06-01-03 | 06-01 | 0 | TEST-01..04 | Exact 47-task, 18-plan, checkpoint, inverse-requirement, command, dependency-topology, Wave 0 file, and cross-plan symbol/fact-kind ownership map | `.tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_validation_map.py --plan-contract && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_acceptance.py --registry-only` | ⬜ pending |
...
| 06-16-01 | 06-16 | 6 | TEST-01, TEST-02 | Schema-discriminated V2 benchmark lock preserves V1 history and binds state repository/parent, source/workflow, nomination, selection, and receipt recovery chain | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_acceptance_domain.py tests/test_operations_state.py -k 'benchmark_lock or manifest_v2 or schema_version or approval_receipt or recovery or selection_chain or rebuild' -x` | ⬜ pending |
| 06-16-02 | 06-16 | 6 | TEST-01, TEST-02 | State-only nomination/lock proves fresh manifest-to-nomination entry equality and fixed-host environment-A approval before one lock persistence | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_phase6_workflow.py tests/test_phase6_source_execution.py tests/test_cli_security.py -k 'fresh_campaign or benchmark_lock or manifest_v2 or recovery or selection_chain or protected_environment or state_only' -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_source_execution.py` | ⬜ pending |
| 06-16-03 | 06-16 | 6 | TEST-01, TEST-02 | checkpoint:human-verify; predecessor 06-16-02; benchmark-only environment-A lock, never a live-authority gateway | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_phase6_workflow.py tests/test_phase6_source_execution.py -k 'benchmark_lock or manifest_v2 or recovery or selection_chain or protected_environment or state_only' -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_source_execution.py` | ⬜ pending |
...
| 06-18-01 | 06-18 | 8 | TEST-01, TEST-02, TEST-04 | Isolated source/topology verifier fixtures reject protected-route and dependency graph mutations before workflow bytes change | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_source_execution.py tests/test_phase6_validation_map.py -k 'fixture or protected_authority or dependency_topology or missing_dependency or cycle or wave' -x` | ⬜ pending |
| 06-18-02 | 06-18 | 8 | TEST-01, TEST-02, TEST-04 | Sole final workflow mutation installs environment-B state-only authority route and final post-freeze bytes | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_workflow.py tests/test_phase6_source_execution.py -k 'record_live_authority or protected_environment or approval_history or no_actor or no_authority_json or locked_source_execution' -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_source_execution.py && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_validation_map.py --plan-contract` | ⬜ pending |
| 06-18-03 | 06-18 | 8 | TEST-01, TEST-02, TEST-04 | checkpoint:human-verify; predecessor 06-18-02; post-ingest verifier 06-08-01; environment-B approval/rebuild is the sole fresh live-authority gateway | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_phase6_workflow.py tests/test_phase6_source_execution.py tests/test_phase6_validation_map.py -k 'live_authority_v2 or protected_environment or approval_receipt or no_semantic or no_publication or checkpoint_topology' -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_validation_map.py --plan-contract` | ⬜ pending |
| 06-08-01 | 06-08 | 9 | TEST-01, TEST-02, TEST-03 | Automatic V2-only re-admission after 06-18-03; historical locks, benchmark-only approval, or GSD acknowledgement cannot open credentials | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_phase6_workflow.py -k 'live_authority or exact_manifest or deepseek_only or no_catalog' -x` | ⬜ pending |
| 06-08-02 | 06-08 | 9 | TEST-01, TEST-02, TEST-03 | Five fixed-SHA live terminal facts | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'benchmark or five_repositories or evaluator_blind or terminal or telemetry' -x` | ⬜ pending |
| 06-08-03 | 06-08 | 9 | TEST-01, TEST-02, TEST-03 | Exact semantic replay zero effects persisted as immutable pre-publication intent; post-publication completion remains separate | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'replay or idempotent or zero_effect' -x` | ⬜ pending |
| 06-09-01 | 06-09 | 10 | TEST-03 | checkpoint:decision; predecessor 06-08-03; post-ingest verifier 06-09-02; exact changed-lineage approval | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_phase3_pipeline.py -k 'prior_lineage or changed_source or approval' -x` | ⬜ pending |
| 06-09-02 | 06-09 | 10 | TEST-03 | Changed source creates new authority and immutable pre-publication intent with no publication effect; Plan 06-18 Task 02 final workflow bytes remain frozen | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_phase3_pipeline.py -k 'changed_source or prior_lineage or new_authority or no_publication' -x` | ⬜ pending |
| 06-10-01 | 06-10 | 11 | TEST-03, TEST-04 | checkpoint:decision; predecessor 06-09-02; post-ingest verifier 06-10-02; post-06-18 frozen-workflow source verifier required | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_source_execution.py tests/test_publication_security.py tests/test_gate_b4_canary.py tests/test_gate_b4_canary_workflow.py tests/test_phase6_acceptance.py -k 'source_execution or publish_workflow or locked_source_execution or preflight or binding or publication_authority' -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_source_execution.py` | ⬜ pending |
| 06-10-02 | 06-10 | 11 | TEST-03, TEST-04 | Fresh causal denials persisted/rebuilt on state branch | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_gate_b4_canary.py tests/test_gate_b4_canary_workflow.py tests/test_phase6_acceptance.py -k 'gate_b4 or causal or stale or cleanup_manifest' -x` | ⬜ pending |
| 06-10-03 | 06-10 | 11 | TEST-03, TEST-04 | One Draft, zero-effect replay, same-Draft update, then distinct typed completion persistence/projection/export/rebuild | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_publication_recovery.py tests/test_publication_security.py -k 'value_draft or publication_replay or changed_source or same_draft or forbidden' -x` | ⬜ pending |
| 06-11-01 | 06-11 | 12 | TEST-02, TEST-04 | checkpoint:human-verify; predecessor 06-10-03; post-ingest verifier 06-11-02; exact-head content verdict and no workflow edit | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'human_review or exact_head or checklist or draft_state' -x` | ⬜ pending |
| 06-11-02 | 06-11 | 12 | TEST-02, TEST-04 | Pre-finalized read-only attestation job; canonical reconciliation | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'attestation or reconcile or submitted_review or stale_head or no_publication' -x` | ⬜ pending |
| 06-11-03 | 06-11 | 12 | TEST-02, TEST-04 | Advice-only strict `ReviewerCalibrationV1` persisted/rebuilt through `acceptance_reviewer_calibration` | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'calibration or agreement or kappa or reviewer_advisory or label_leakage' -x` | ⬜ pending |
| 06-12-01 | 06-12 | 13 | TEST-04 | checkpoint:human-action; predecessor 06-11-03; post-ingest verifier 06-12-02; separate probe-only cleanup | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_gate_b4_canary.py tests/test_phase6_acceptance.py -k 'cleanup or probe_only or value_draft_untouched' -x` | ⬜ pending |
| 06-12-02 | 06-12 | 13 | TEST-04 | Pre-finalized read-only cleanup-attestation job; no workflow edit | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py tests/test_gate_b4_canary_workflow.py -k 'cleanup_attestation or exact_targets or no_cleanup_route or value_draft' -x` | ⬜ pending |
| 06-13-01 | 06-13 | 14 | TEST-01..04 | Final hard-gate/map mutation coverage independently requires both typed post-publication completion facts and rejects intent-only credit | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'verifier or validation_map or mutation or read_only or all_44' -x` | ⬜ pending |
| 06-13-02 | 06-13 | 14 | TEST-01..04 | Deterministic report/all-44 rebuild consumes independently rebuilt publication completion identities/effects/lineage/PR-head facts | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_acceptance.py -k 'report or requirement_map or rebuild or warning or recommendation' -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_acceptance.py` | ⬜ pending |
| 06-13-03 | 06-13 | 14 | TEST-01..04 | Final Nyquist statuses from executed evidence | `.tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_validation_map.py && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_acceptance.py` | ⬜ pending |
| 06-14-01 | 06-14 | 15 | TEST-01..04 | Provider/architecture/operator docs | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_semantic_provider.py tests/test_phase6_workflow.py tests/test_phase6_acceptance.py -k 'docs or provider or command or boundary' -x && git diff --check` | ⬜ pending |
| 06-14-02 | 06-14 | 15 | TEST-01..04 | Testing/release posture matches report | `.tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_validation_map.py && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_acceptance.py && git diff --check` | ⬜ pending |
| 06-14-03 | 06-14 | 15 | TEST-01..04 | Read-only structural source-execution verifier over the post-06-18 protected workflow routes plus mutation suite; fresh offline wheel equals sole installed/documented SHA-256 | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_phase6_source_execution.py -x && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_source_execution.py && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_acceptance.py && git diff --quiet -- .github/workflows/discover.yml .github/workflows/publish-candidate.yml .github/workflows/gate-b4-canary.yml .github/workflows/phase6-acceptance.yml && git diff --cached --quiet -- .github/workflows/discover.yml .github/workflows/publish-candidate.yml .github/workflows/gate-b4-canary.yml .github/workflows/phase6-acceptance.yml && .tools/uv-0.11.29/bin/uv --no-cache --offline lock --check && build_stage="$(mktemp -d)" && trap 'rm -rf "$build_stage"' EXIT && .tools/uv-0.11.29/bin/uv --no-cache build --offline --no-sources --wheel --out-dir "$build_stage" . && .tools/uv-0.11.29/bin/uv run --locked python -c 'import hashlib,re,sys; from pathlib import Path; stage=Path(sys.argv[1]); expected="skillscout-0.1.0-py3-none-any.whl"; built=list(stage.iterdir()); published=list(Path("dist").glob("skillscout-*.whl")); assert len(built)==1 and built[0].is_file() and built[0].name==expected, built; assert len(published)==1 and published[0].is_file() and published[0].name==expected, published; release=Path("RELEASE.md").read_text(encoding="utf-8"); documented=re.findall(r"Release wheel SHA-256: `([0-9a-f]{64})`", release); assert len(documented)==1, documented; digest=lambda path: hashlib.sha256(path.read_bytes()).hexdigest(); assert digest(built[0])==digest(published[0])==documented[0]; assert "release-document evidence only" in release and "Gate B4 independently binds" in release' "$build_stage" && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_validation_map.py && .tools/uv-0.11.29/bin/uv run --locked pytest -q && .tools/uv-0.11.29/bin/uv run --locked ruff check . && git diff --check && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_source_execution.py && .tools/uv-0.11.29/bin/uv run --locked python tools/verify_phase6_acceptance.py` | ⬜ pending |
...
|-------------|---------------------|
| TEST-01 | 06-01-01, 06-01-02, 06-01-03, 06-02-01, 06-03-01, 06-03-02, 06-04-01, 06-04-02, 06-05-01, 06-05-02, 06-07-01, 06-07-02, 06-07-03, 06-16-01, 06-16-02, 06-16-03, 06-17-01, 06-17-02, 06-17-03, 06-18-01, 06-18-02, 06-18-03, 06-08-01, 06-08-02, 06-08-03, 06-13-01, 06-13-02, 06-13-03, 06-14-01, 06-14-02, 06-14-03, 06-15-01 |
| TEST-02 | 06-01-01, 06-01-03, 06-02-01, 06-02-02, 06-02-03, 06-04-01, 06-04-02, 06-06-01, 06-06-02, 06-06-03, 06-07-01, 06-07-02, 06-07-03, 06-16-01, 06-16-02, 06-16-03, 06-17-01, 06-17-02, 06-17-03, 06-18-01, 06-18-02, 06-18-03, 06-08-01, 06-08-02, 06-08-03, 06-11-01, 06-11-02, 06-11-03, 06-13-01, 06-13-02, 06-13-03, 06-14-01, 06-14-02, 06-14-03 |
| TEST-03 | 06-01-01, 06-01-03, 06-02-01, 06-04-01, 06-04-02, 06-05-01, 06-05-02, 06-06-01, 06-06-02, 06-06-03, 06-17-01, 06-17-02, 06-17-03, 06-08-01, 06-08-02, 06-08-03, 06-09-01, 06-09-02, 06-10-01, 06-10-02, 06-10-03, 06-13-01, 06-13-02, 06-13-03, 06-14-01, 06-14-02, 06-14-03, 06-15-01 |
| TEST-04 | 06-01-01, 06-01-02, 06-01-03, 06-02-01, 06-02-02, 06-02-03, 06-03-01, 06-03-02, 06-04-01, 06-04-02, 06-05-01, 06-05-02, 06-06-01, 06-06-02, 06-06-03, 06-18-01, 06-18-02, 06-18-03, 06-10-01, 06-10-02, 06-10-03, 06-11-01, 06-11-02, 06-11-03, 06-12-01, 06-12-02, 06-13-01, 06-13-02, 06-13-03, 06-14-01, 06-14-02, 06-14-03, 06-15-01 |
...
| `BenchmarkEntryV1` | strict historical model | 06-04-01 | 06-07-02, 06-07-03 only |
| `NominationSetV1` | strict historical model | 06-04-01 | 06-07-02, 06-07-03 only |
| `BenchmarkLockAttestationV1` | strict historical attestation | 06-04-01 | 06-07-03 only |
| `LockedBenchmarkManifestV1` | strict historical model | 06-04-01 | 06-07-03 only |
| `BenchmarkLockApprovalReceiptV2` | strict redacted approval receipt with `purpose=benchmark_lock` | 06-16-01 | 06-16-02, 06-16-03, 06-17-01, 06-17-02 |
| `LockedBenchmarkManifestV2` | strict fresh lock fact; binds state repository/parent, source/workflow, static V1 selection-manifest, fresh nomination, selected entries, and V2 approval receipt | 06-16-01 | 06-16-02, 06-16-03, 06-17-01, 06-17-02, 06-17-03, 06-18-02, 06-18-03, 06-08-01 |
| `LiveAcceptanceAuthorityV1` | strict historical model | 06-04-01 | 06-17-01 historical restore only |
| `LiveAuthorityApprovalReceiptV2` | strict redacted approval receipt with `purpose=live_execution` | 06-17-01 | 06-17-02, 06-17-03, 06-18-02, 06-18-03, 06-08-01 |
| `LiveAcceptanceAuthorityV2` | strict fresh live-authority fact bound to complete V2 lock chain and distinct approval receipt | 06-17-01 | 06-17-02, 06-17-03, 06-18-02, 06-18-03, 06-08-01 |
...
| `OperationsStateStore.record_acceptance_fact` | typed persistence with schema-version dispatch | 06-04-02 | 06-06-03, 06-07-02, 06-16-01, 06-16-02, 06-17-01, 06-18-02 |
| `OperationsStateStore.acceptance_snapshot` | typed projection with historical V1 and fresh V2 facts | 06-04-02 | 06-06-03, 06-16-01, 06-16-02, 06-17-01, 06-17-02, 06-18-02, 06-13-02 |
| `export_owned_state` | canonical acceptance export extension | 06-04-02 | 06-06-03, 06-16-01, 06-17-01, 06-18-02, 06-13-02 |
| `rebuild_owned_state` | typed acceptance rebuild extension | 06-04-02 | 06-06-03, 06-16-01, 06-17-01, 06-17-02, 06-18-03, 06-13-02 |
...
| `NominationDependencies` | capability dataclass | 06-05-01 | 06-07-02, 06-16-02 |
| `LockedCampaignDependencies` | capability dataclass | 06-05-01 | 06-16-02, 06-17-02, 06-08-02 |
...
| `prepare-fresh-campaign` | state-only CLI command | 06-16-02 | 06-16-03 |
...
| `lock_fresh_campaign` | workflow job/action | 06-16-02 | 06-16-03, 06-17-01 |
| `skillscout-phase6-benchmark-lock` | protected workflow job | 06-16-02 | 06-16-03 |
| `record_live_authority` | workflow job/action | 06-18-02 | 06-18-03, 06-08-01 |
...
- [ ] `tests/test_phase6_source_execution.py` freezes the exact four-workflow source-only grammar and every forbidden acquisition/invocation/order mutation, including the later `prepare_fresh_campaign`, `lock_fresh_campaign`, and `record_live_authority` routes, before the final Plan 06-18 verifier turns it green.
...
- [ ] `tools/verify_phase6_validation_map.py` parses all 18 plans and proves 47 unique primary task rows, checkpoint dependencies, TEST-01..TEST-04 forward/inverse coverage, commands, and this complete Wave 0 file set.
- [ ] `tools/verify_phase6_validation_map.py` proves every dependency target exists, the plan graph is acyclic, every predecessor has a lower wave, and each plan wave is exactly zero for a root or one greater than its highest predecessor wave; isolated-tree mutation tests cover missing dependency, cyclic edge, same/future-wave predecessor, and wrong derived wave.
...
|----------|-------------|------------|-------------------|
| Lock the five-repository benchmark | TEST-01 | Repository identity, license, outcome-role mix, and suitability require accountable human selection before credentials or LLM calls | Select and commit the exact static V1 `.planning/phases/06-adversarial-mvp-acceptance/06-BENCHMARK-MANIFEST.json`; configure `phase6-human-benchmark-lock` with required reviewer `alexzhu0`, Prevent self-review disabled for the sole reviewer, and no custom protection rule; approve the state-only lock job so its V2 `benchmark_lock` receipt binds the manifest digest without any comment evidence |
| Approve fresh live authority | TEST-01, TEST-02, TEST-04 | Permission to resolve live credentials is distinct from selecting the benchmark and must be externally auditable | Configure `skillscout-phase6-live-authority` with required reviewer `alexzhu0`, Prevent self-review disabled for the sole reviewer, and no custom protection rule; approve only the waiting state-only `record-live-authority` job and confirm a V2 `live_execution` receipt bound to the V2 lock, without a comment, actor assertion, semantic call, or publication effect |
...
- [ ] Protected live/manual evidence is current and bound to exact identities, heads, workflows, policies, and purpose-distinct human approval receipts; 06-18-03 is the only fresh human/platform gateway for 06-08-01.
- [ ] The dedicated source-execution verifier parses exact discover/publish/canary/Phase6 workflows, finds every authoritative SkillScout step including `prepare_fresh_campaign`, `lock_fresh_campaign`, and `record_live_authority`, proves same-job full-SHA checkout/pinned local materialization/direct checkout-local locked invocation order, and mutation tests reject every non-checkout, non-locked, registry, wheel/dist, artifact, preinstalled-command, alias/function/variable/wrapper, indirect, external-working-directory, empty-scan, and unknown route.
- [ ] Plan 06-18 Task 02 is the final permitted workflow-byte mutation; every later source verifier, preflight, SHA-256 calculation, and Gate B4 binding consumes only its post-task workflow bytes.
