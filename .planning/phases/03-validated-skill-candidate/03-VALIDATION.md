...
| 03-10-03 | 03-10 | 9 | GEN-04, GEN-05, VAL-03, REV-03 | T-03-29, T-03-30 | Attestation owns raw review evidence plus reviewer-specific retry policy, maximum attempts, exact attempt count, and bounded transient-failure history; terminal owns eligibility policy, lineage/review statuses, Generator evidence, and exact branch matrix | attestation unit | `sh tools/verify_phase3_gate_b3.sh && UV_PYTHON_INSTALL_DIR="$PWD/.tools/python" UV_MANAGED_PYTHON=1 UV_PYTHON_DOWNLOADS=never "$PWD/.tools/uv-0.11.29/bin/uv" run --locked pytest -q tests/test_openai_review.py` | `tests/test_openai_review.py` | ⬜ pending |
