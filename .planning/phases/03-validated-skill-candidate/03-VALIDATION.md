...
# status lifecycle: draft (seeded by plan-phase) → validated (set after execution verification)
status: validated
...
| **Quick run command** | `sh tools/verify_phase3_gate_b3.sh && UV_PYTHON_INSTALL_DIR="$PWD/.tools/python" UV_MANAGED_PYTHON=1 UV_PYTHON_DOWNLOADS=never "$PWD/.tools/uv-0.11.29/bin/uv" run --locked pytest -q tests/test_phase3_bootstrap.py tests/test_candidate_source.py tests/test_candidate_authority.py tests/test_qualification.py tests/test_skill_generation.py tests/test_skill_validation.py tests/test_openai_review.py tests/test_phase3_pipeline.py` |
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 03-01-01 | 03-01 | 1 | VAL-01 | T-03-SC, T-03-01, T-03-02 | Gate A3 reviews exact `skills-ref==0.1.1` identity, audited wheel hash, and anomalies before resolution | human checkpoint | N/A — this is a non-auto-approvable human supply-chain decision. | N/A | ✅ green |
...
- [x] All 29 tasks have an automated verification command or non-auto-approvable checkpoint.
- [x] All 13 Phase 3 requirement IDs appear explicitly in plan frontmatter and this map.
- [x] No unresolved symbolic path tokens, ellipses, requirement ranges, or shorthand commands remain.
- [x] Every post-B3 uv/import/test/Ruff/validator command starts with `sh tools/verify_phase3_gate_b3.sh &&`.
- [x] Gate A3 and Gate B3 are recorded as blocking-human and cannot auto-advance.
- [x] Sampling continuity has no unvalidated implementation task.
- [x] No watch-mode flags or live-network dependency.
- [x] Dependency-free validation-map checker and its temporary-copy mutation suite pass before release credit.
- [x] The validation-map rows and Requirement Coverage inverse exactly match all source PLAN verification contracts.
- [x] The checker safely admits all 14 PLAN files and this map, rejects malformed or duplicate tables, and allows only its hard-coded canonical command sequences.
- [x] Parity-preserving logical-OR, separator, pipe, newline/control, substitution, redirection, process-substitution, heredoc, comment, and escape mutations fail before any mapped command could run.
- [x] Full suite command passes without changing `uv.lock`, as proven by the terminal Gate B3 postflight after pytest.
- [x] The full suite includes a fresh Gate B3 preflight immediately before repository-local `uv build --no-sources`.
- [x] `nyquist_compliant: true` is set only after Phase 3 execution verification passes.

**Approval:** validated — Gate A3 and Gate B3 were explicitly approved; Phase 3 verification passed 13/13 with the exact release chain.
