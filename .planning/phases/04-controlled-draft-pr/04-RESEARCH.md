...
|----|-------------|------------------|
| PUB-01 | Publisher 只向配置的受控中央 Skill catalog 仓库创建或更新确定性机器分支、提交已验证 artifact、创建 Draft Pull Request，并请求配置的人类 reviewer 或 team。 | Satisfy the requirement's human-review destination through configured individual reviewer logins: use catalog-bound authority, deterministic branch identity, a manifest-gated Git Data transaction, `draft: true`, and the users-only review-request body. Team targets are outside the safe v1 evidence contract and fail closed before token/network. [VERIFIED: codebase grep and Plans 04-02–04-06] [CITED: https://docs.github.com/en/rest/git] [CITED: https://docs.github.com/en/rest/pulls/pulls] [CITED: https://docs.github.com/en/rest/pulls/review-requests] |
...
Request individual reviewers through `POST /pulls/{number}/requested_reviewers` with only the `reviewers` array. For recovery, fully paginate GET requested reviewers for pending users and List reviews for completed users. GitHub documents that a user disappears from requested reviewers after submitting a review and is then represented by List reviews. Team input and non-empty provider team state fail closed to manual catalog review. [CITED: https://docs.github.com/en/rest/pulls/review-requests]
...
| Approve | No review-submission method or `POST /pulls/{n}/reviews`; isolated transport/static tests prove the production adapter/CLI/workflow cannot express approval. [CITED: https://docs.github.com/en/pull-requests/how-tos/review-pull-requests/approving-a-pull-request-with-required-reviews] | Do not claim a general token-scope denial; required human approval remains unsatisfied and automation has no production route. |
| Ready for review / auto-merge | No GraphQL transport or Draft-transition method; isolated transport/static tests prove the production adapter/CLI/workflow cannot express the transition. [VERIFIED: recommended design] | Do not claim token scopes deny ready-for-review; `Pull requests: write` is coarse and may retain residual capability outside SkillScout. [VERIFIED: permission analysis] |
...
**Authority rationale:** The achievable defense-in-depth contract is intentionally asymmetric. Live platform evidence covers default-branch push, merge, ruleset/administration, unauthorized repository/resource, and secret access. Approval and ready-for-review are enforced as absent production capabilities through exact route/transport/CLI/workflow allowlists. The residual risk is that a stolen or separately used `Pull requests: write` token may be able to transition a Draft; this is not accepted as SkillScout authority, must be recorded in the live gate, and any production call path for that transition blocks release. [VERIFIED: GitHub permission decomposition]
...
| Review notification | Custom comments, mentions, or inferred team receipts | Users-only requested-reviewers endpoint plus current-request and List reviews observations | GitHub supplies durable-enough pending/completed evidence for individual users; official timeline fields do not promise team identity. [CITED: https://docs.github.com/en/rest/pulls/review-requests] [CITED: https://docs.github.com/en/rest/using-the-rest-api/issue-event-types] |
...
**How to avoid:** Support only sorted unique individual logins. On a new PR request once and verify current state. On recovery, combine fully paginated current requests with List reviews; if a configured login is in neither set, including removed-after-request, stop for manual intervention rather than re-notify. Reject team configuration before token/network. [CITED: https://docs.github.com/en/rest/pulls/review-requests]
...
2. **Which individual human logins can be requested safely and recovered deterministically?**
   ...
   - Resolution: v1 supports only a non-empty sorted unique tuple from `SKILLSCOUT_CATALOG_REVIEWERS`. `SKILLSCOUT_CATALOG_TEAM_REVIEWERS` must be absent/empty; any team target fails before token/network and requires human/manual catalog review. Recovery uses fully paginated current requested users plus List reviews. Pending or completed users are not re-notified; a recovered configured login in neither set, including removed-after-request, yields manual intervention. Only a newly created PR with no prior request opportunity may request a previously unseen login. [VERIFIED: Plans 04-02, 04-03, 04-04, 04-05, 04-06, and 04-10]
...
5. **How will the merge canary isolate ruleset denial from Draft/check/conflict denial?**
   ...
   - Resolution: Plan 09's isolated canary-only client requires an otherwise mergeable disposable PR, independently exported active rules/bypass evidence, same-installation attestation, bounded provider classification, and pre/post default SHA equality for default-write/merge plus ruleset/administration/unauthorized-resource/secret-resource probes. Plan 10 checkpoint `04-10-01` reviews causal prerequisites and unchanged-state evidence. Approve and ready-for-review are instead covered by isolated negative transport/static evidence proving the production adapter, CLI, and workflow have no such route; the residual coarse-token ready capability is recorded explicitly. Ambiguous live denial or unavailable causal setup blocks acceptance. [VERIFIED: Plans 04-09 and 04-10]
...
- [Review requests](https://docs.github.com/en/rest/pulls/review-requests) — current users/teams, request semantics, and transition from requested state to List reviews. [CITED: https://docs.github.com/en/rest/pulls/review-requests]
- [Issue event types](https://docs.github.com/en/rest/using-the-rest-api/issue-event-types) — exact `review_requested`/`review_request_removed` event values and person-only `requested_reviewer` property. [CITED: https://docs.github.com/en/rest/using-the-rest-api/issue-event-types]
- [Timeline events](https://docs.github.com/en/rest/issues/timeline) — installation-token support with Pull requests read; not used as team identity authority. [CITED: https://docs.github.com/en/rest/issues/timeline]
