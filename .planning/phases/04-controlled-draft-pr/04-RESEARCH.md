...
2. **Which human users and/or team slugs must be requested?**
   ...
   - Resolution: Protected `SKILLSCOUT_CATALOG_REVIEWERS` and `SKILLSCOUT_CATALOG_TEAM_REVIEWERS` hold bounded lists; at least one combined target is mandatory. Plan 10 checkpoint `04-10-01` verifies the configured identities exist and are authorized. Reconciliation combines current user/team requests, completed user reviews, and a bounded exact review-request timeline that preserves team slugs across assignment, fulfillment, and removal. Missing/malformed identities or ambiguous, truncated, conflicting, reordered, manually removed, or unbound assignment evidence blocks publication instead of repeating a team notification. [VERIFIED: Plans 04-02, 04-04, 04-05, 04-06, and 04-10]
