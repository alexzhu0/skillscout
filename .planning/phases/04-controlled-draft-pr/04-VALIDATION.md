...
| PUB-04 | T-04-05, T-04-06, T-04-14 | Unprivileged output is candidate-only; exact `verify-publication-admission --compare-env` revalidates it and derives protected-local intent/admission before token; same-identity live canary denies default write, merge, ruleset/admin, unauthorized resource, and secret access | workflow static + mutation + opt-in live integration | `.tools/uv-0.11.29/bin/uv run --locked pytest -q tests/test_publication_live_canary.py tests/test_publication_security.py -x` | ❌ W0 | ⬜ pending |
